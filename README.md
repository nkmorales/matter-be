...
This is a boilerplate to build a full stack web application using MongoDB, Express, React, Node.js, and Webpack. It is also configured with webpack-dev-server, eslint, esm, prettier and babel.
...
git clone https://github.com/rguan72/mern-boilerplate
...
There is a client and server directory. All the frontend code (react, css, js and any other assets) will be in client directory. Backend Node.js/Express code will be in the server directory. Client and server source code is contained in their own `src` folders, respectively. All static files will be built into `bundle.js`, contained in `client/dist` for the production server.
...
[esm](https://github.com/standard-things/esm) is used to transpile server side code to ECMAScript without using babel. `node -r esm index.js` runs the file `index.js` using ESM.
